...
//                nfh_order_process_logging( $order_id, 'log: updated user order name to serial and key' );
                ...
                if (WC()->cart->cart_contents[$cart_item_key]['del_method'] == 'Email') {
                    ...
                    send_1FORYOU_voucher_email(WC()->cart->cart_contents[$cart_item_key]['del_value'], $purchase['voucher']['pin'], $purchase['voucher']['expiryDate'], $purchase['voucher']['serialNumber'], $voucher_amount);

                } else if (WC()->cart->cart_contents[$cart_item_key]['del_method'] == 'Number') {

                    nfh_order_process_logging( $order_id, '5. Attempting to send SMS to '. WC()->cart->cart_contents[$cart_item_key]['del_value']);

                    send_1FORYOU_voucher_sms(WC()->cart->cart_contents[$cart_item_key]['del_value'], $purchase['voucher']['pin'], $purchase['voucher']['expiryDate'], $purchase['voucher']['serialNumber'], $voucher_amount);
    ...
    <div class="prod-info-popup" data-popup="<?php echo $prod_tag[0]->term_id; ?>" >
        ...
        <div class="product-info-column">
            ...
            <form class="prodAddForm" method="POST" action="">
                ...
                $args = array('post_type' => 'product', 'order' => 'ASC', 'product_tag' => $prod_tag[0]->slug);
                ...
                while ($loop->have_posts()) : $loop->the_post();
                    ...
                    if(!$has_custom_price) { ?>
                        ...
                        <label for="<?php echo $name; ?>"><span>
                        <?php echo $product->get_price(); ?></span></label>
                    <?php } else {
